...
def load_status():
    """Load current trial status with caching for stability."""
    if 'last_valid_status' not in st.session_state:
        st.session_state.last_valid_status = None
        
    if not Path(STATUS_FILE).exists():
        return st.session_state.last_valid_status

    try:
        with open(STATUS_FILE, 'r') as f:
            status = json.load(f)
            st.session_state.last_valid_status = status
            return status
    except:
        # If read fails (lock), return last known good status to prevent flashing
        return st.session_state.last_valid_status
